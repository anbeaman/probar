...
- **HTTP 传输层韧性增强**(`pb.core.http`,应对东财反爬下的取数稳定性,纯稳健 + 友好访问、非绕过):
  - **指数退避 + 随机抖动**重试(原固定 3 次线性退避 → 5 次指数退避 `0.5→8s` + `0~50%` 抖动),riding out 东财 `push2` 常见的**瞬时断连**(`Server disconnected without sending a response`)。
  - **按 host 熔断**:某域连续整通断连(`push2` 突发后 IP 级封禁的特征)后短时熔断(冷却 60s),熔断窗口内对该域调用**快速失败**而非继续捶打——硬刚只会空耗重试并延长封禁。
  - `429` 改为先退避重试、穷尽才抛 `RateLimited`。
  - 浏览器级请求头(`Accept-Language` / `Connection` / `Sec-Fetch-*`)。
  - **默认请求速率 10 → 5 req/s**(更友好,预防突发触发封禁;仍可经 `EastMoney(rate=…)` 调高)。非破坏性。

### 说明(东财反爬的边界,实测)

- 东财 `push2.eastmoney.com`(实时快照 / 批量 / 全市场扫描 `clist`)**对突发请求会 IP 级封禁数分钟**,期间该域所有端点断连;`push2his`(K 线)/ `datacenter`(龙虎榜、财务)不受影响。
- **无客户端魔法绕过**:编号镜像(`1..N.push2`)同 IP 同封;延迟镜像 `push2delay` 虽可连但**不尊重 `fs` 过滤**(查股票却返回板块/基金),数据错配,已否决作为 fallback。稳定取数的正解是**控频 + 退避 + 熔断 + 复用缓存**(`securities()` 已带 TTL 缓存),被封后等冷却而非硬刚。
