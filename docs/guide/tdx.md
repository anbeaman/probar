...
## `ticks_hist` —— 历史逐笔成交(v0.7,已实现)

```python
df = pb.tdx.ticks_hist("600519.SH", date="2026-06-18")            # 该交易日全部逐笔
df = pb.tdx.ticks_hist("600519.SH", date="20260618", limit=100)   # 只要最新 100 笔
```

- **参数**:`date` = 交易日(`"YYYY-MM-DD"` 或 `"YYYYMMDD"`);`limit` = 最多笔数(默认 `None` 取全天,给值取最新 N 笔)。
- **返回列**:`symbol, date, time(HH:MM), price(元), vol(手), buyorsell`。
- **与当日逐笔的差异**:历史协议**不返回 `num`(笔数)**,故比 `ticks` 少一列、多一列 `date`;
  `buyorsell` 仍为通达信原值(常见 0 买 / 1 卖 / 2 中性,集合竞价等为特殊值)。
...
| `block` | ✅ v3.1 | 板块及成分股(概念/风格/指数;文件协议) |
