...
| `pb.tdx` | 通达信 | 行情底座:批量实时五档、历史分钟、**历史逐笔**、除权除息、股本快照 |
...
# 通达信(自写二进制协议,纯标准库):批量五档 / 复权K线 / 逐笔 / 股本快照
q = pb.tdx.quotes(["000001.SZ", "600519.SH"])           # 批量实时五档
k = pb.tdx.kline("600519.SH", freq="1m", limit=240)      # 历史分钟(比东财更深)
t = pb.tdx.ticks_hist("600519.SH", date="2026-06-18")    # 历史逐笔(通达信独有)
f = pb.tdx.finance_info("600519.SH")                     # 流通股本 / 每股净资产
```
